...
export const DEFAULT_AVATAR_ID = DEFAULT_PLAYER_AVATAR_ID;
...
/** opponentIndex: 0 = first AI seat, 1 = second AI seat (고스톱 3P). */
export function getOpponentAvatarId(
  ...
  const taken = new Set<AvatarId>([playerAvatarId, aiAvatarId]);
  const alternate = AVATAR_IDS.find((id) => !taken.has(id));
  return alternate ?? AVATAR_IDS[(AVATAR_IDS.indexOf(aiAvatarId) + 1) % AVATAR_IDS.length];
}
